...
if (isset($_POST['compute'])){
    ...
    for($i = 0; $i < $N; $i++)
    {
            ...
            if($ret == 0){  //if no record exists, insert a record
                ...
                $query=mysqli_query($con,"insert into studentres(idnumber,gradelvl,semester,sy,subject,unit,score,dateAdded) 
                value('$matricNo','$levelId','$semesterId','$sessionId','$subject[$i]','$unit[$i]','$score[$i]','$dateAdded')");
           ...
           //Checks if result has been computed (MatricNo, level, semester and session)
            ...
            if($ret > 0){
                ...
                $alertStyle ="alert alert-danger";
                $statusMsg="The result has been computed for this student for this semester!";
            }
            else{

                $querys = mysqli_query($con,"insert into tblfinalresult(matricNo,levelId,semesterId,sessionId,dateAdded) 
                value('$matricNo','$levelId','$semesterId','$sessionId','$dateAdded')");
            }
                    ...
                    <div class="col-md-12">
                        <div class="card">
                            <div class="card-header">
                                <strong class="card-title"><h4 align="center"><?php echo  $rowStd['firstName'].' '.$rowStd['lastName']?>'s&nbsp;<?php echo $rowLevel['levelName'];?>&nbsp;<?php echo '<strong style="color:red;">['.$rowSemester['semesterName'].']</strong>';?> - Result</h></strong>
                            </div>
                            <form method="post">
                            <div class="card-body">
                                <p id="demo"></p>
                             <div class="<?php if(isset($alertStyle)){echo $alertStyle;}?>" role="alert"><?php if(isset($statusMsg)){echo $statusMsg;}?></div>
                                <table class="table table-hover table-striped table-bordered">
                                    ...
                                    <?php 
                                        $sql3 = mysqli_query($con, "SELECT * FROM studentsub WHERE name = '$name' AND semester = $semesterId");
