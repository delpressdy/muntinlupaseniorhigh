...
if(isset($_POST['submit'])){
  ...
  $dateCreated = date("Y-m-d");
    ...
    if($res > 0){
      ...
      $statusMsg="Teacher with the User ID already exist!";
    ...
    $alertStyle ="alert alert-danger";
    $statusMsg="Teacher has been Assigned to this Section!";

    }
    else{

    $query=mysqli_query($con,"insert into tbladmin(firstName,lastName,otherName,emailAddress,phoneNo,password,staffId,adminTypeId,isPasswordChanged,isAssigned,dateCreated) value('$firstname','$lastname','$othername','$emailAddress','$phoneNo','123','$staffId','2','0','0','$dateCreated')");

    if($query) {

        $queryss=mysqli_query($con,"insert into tblassignedadmin(name,staffId,departmentId,facultyId,dateAssigned) value('$firstname','$staffId','$departmentId','$facultyId','$dateAssigned')");
        if($queryss){
            ...
            if($querys == True){
                ...
                $statusMsg="Teacher Created and Assigned Successfully!";
            }
            else
            {
                $alertStyle ="alert alert-danger";
                ...
                <br><br>
                    <div class="col-md-12">
                        <div class="card">
                            ...
                            <?php
                                $sql = "SELECT * FROM tbladmin WHERE adminTypeId = 2";
                                $res = $con->query($sql);
